...
def aiamaps(directory, save_directory, submap=None, cmlims = [], rectangle=[], save_inc=True, iron='',
    ...
    for f in range(no_of_files):
        ...
        #now remove negative values for a log scale        
        if cm_scale == 'LogNorm':
            m_data = smap.data
            m_data[m_data<=0] = np.min(np.min(m_data[m_data>0])) #set to min positive value to avoid nans in the log plot for values <=0
            smap = sunpy.map.Map(m_data,smap.meta)
            del m_data  
        
        #change colour map if the channel is iron 16, 18, or a difference map
        if iron == '18':
...
def contourmaps_from_dir(aia_dir, nustar_dir, nustar_file, save_dir, chu='', fpm='', energy_rng=[], submap=[], 
            ...
            save_inc=False, gauss_sigma=4, background_time='begin'):
    ...
    for t in range(len(time_bins)-1):
        ...
        if len(cleanevt) != 0 and aia == ('ns_overlap_only' or 'all'): #AIA data and NuSTAR data
            ...
            dd=ndimage.gaussian_filter(nustar_map.data, gauss_sigma, mode='nearest');
